...
export default function WishListPage() {
  ...
  useEffect(() => {
    ...
    ProductService.getProducts().then((res) => {
      ...
      const filtredProducts = fetchedProducts.filter((product: Product) =>
  ...
  return (
    <main className="flex flex-col items-center justify-center">
      <section className="w-full flex flex-col items-center p-2 md:p-8">
        <h1 className="font-playfair-display text-4xl font-bold">Wishlist</h1>
        {products.length > 0 && count > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 3xl:grid-cols-5 w-full">
